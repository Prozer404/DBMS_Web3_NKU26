-- =============================================================================
-- Web3 交易所数据库：删库后整库重建（MySQL 8.0.16+）
-- 警告：将 DROP DATABASE Web3，该库内全部数据丢失。
-- 用法：执行本文件后，再执行 seed_web3.sql 灌入测试数据。
-- =============================================================================

DROP DATABASE IF EXISTS `Web3`;
CREATE DATABASE `Web3` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE `Web3`;
...
-- -----------------------------------------------------------------------------
-- 6. 触发器：充值成功自动入账并写资金流水
-- -----------------------------------------------------------------------------
DELIMITER $$
CREATE TRIGGER trg_deposits_credit_after_success
AFTER UPDATE ON deposits
FOR EACH ROW
BEGIN
  DECLARE v_balance DECIMAL(20, 8);
  DECLARE v_frozen DECIMAL(20, 8);

  IF NEW.status = 1 AND OLD.status <> 1 THEN
    INSERT INTO assets (user_id, currency, balance, frozen_balance)
    VALUES (NEW.user_id, NEW.currency, NEW.amount, 0.00000000)
    ON DUPLICATE KEY UPDATE balance = balance + NEW.amount;
    ...
    INSERT INTO ledger_entries (
      user_id, currency, amount, balance_after, frozen_balance_after,
      ref_type, ref_id, `type`
    ) VALUES (
      NEW.user_id, NEW.currency, NEW.amount, v_balance, v_frozen,
      'DEPOSIT', NEW.id, 'RECHARGE'
    );
  END IF;
END$$
DELIMITER ;
...
-- -----------------------------------------------------------------------------
-- 7. 存储过程：KYC 审核时同步用户 KYC 状态
-- -----------------------------------------------------------------------------
DELIMITER $$
CREATE PROCEDURE sp_review_kyc(
  IN p_kyc_id BIGINT,
  IN p_status TINYINT,
...
BEGIN
  ...
  DECLARE v_user_kyc_status TINYINT;

  IF p_status NOT IN (0, 1, 2) THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = '非法 KYC 审核状态';
  END IF;
  ...
  IF v_user_id IS NULL THEN
    ROLLBACK;
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'KYC 申请不存在';
  END IF;

  SET v_user_kyc_status = CASE p_status
    WHEN 0 THEN 1
    WHEN 1 THEN 2
    WHEN 2 THEN 3
  END;
  ...
  UPDATE users
     SET kyc_status = v_user_kyc_status
   WHERE id = v_user_id;
...
-- 8. 视图：用户成交查询
-- -----------------------------------------------------------------------------
CREATE OR REPLACE VIEW v_user_trades AS
SELECT
  mo.user_id AS maker_user_id,
  taker.user_id AS taker_user_id,
  t.id AS trade_id,
  t.pair_id,
  tp.base_currency,
  tp.quote_currency,
